...
Function Import-FileShareToSPO {
    ...
    Try {
        ...
        $Source = @($SourceItems | Select-Object FullName, PSIsContainer,
        ...
        $Target = @($TargetFiles | Select-Object @{Label = 'FullName'; Expression = { $_.FieldValues.FileRef.Replace($TargetFolder.ServerRelativeURL, $SourceFolderPath).Replace("/", "\") } },
        ...
        $SourceDelta = @($FilesDiff | Where-Object { $_.SideIndicator -eq "<=" }) 
...
Function Remove-FileShareDeltaInSPO {
    ...
    Try {
        ...
        $Counter = 1
        $FilesDiff = Compare-Object -ReferenceObject $Source -DifferenceObject $Target -Property FullName, PSIsContainer, TargetItemURL, LastUpdated     
        $TargetDelta = @($FilesDiff | Where-Object { $_.SideIndicator -eq "=>" })
